...
public final class FabricTransformer {
	...
	// CatServer start
	public static boolean transform(final org.objectweb.asm.tree.ClassNode classNode, final org.objectweb.asm.Type classType) {
		...
		boolean transformAccess = isMinecraftClass && FabricLauncherBase.getLauncher().getMappingConfiguration().requiresPackageAccessHack();
		boolean environmentStrip = !isMinecraftClass || isDevelopment;
		boolean applyAccessWidener = isMinecraftClass && FabricLoaderImpl.INSTANCE.getAccessWidener().getTargets().contains(name.replace("/", "."));
		...
		if (environmentStrip) {
			...
			classNode.accept(tempWriter);
			ClassReader tempReader = new ClassReader(tempWriter.toByteArray());
			tempReader.accept(stripData, ClassReader.SKIP_CODE | ClassReader.SKIP_FRAMES);
		...
		if (visitorCount <= 0 || visitor == null) {
			return false;
		}

		classNode.accept(visitor);
		// ClassNode#accept() will not change the access of methods, so...
		for (MethodNode methodNode : classNode.methods) {
			methodNode.access = FabricLoaderImpl.INSTANCE.getAccessWidener().getMethodAccess(new net.fabricmc.accesswidener.EntryTriple(classNode.name, methodNode.name, methodNode.desc)).apply(methodNode.access, methodNode.name, classNode.access);
		}
		return true;
	}
	// CatServer end
