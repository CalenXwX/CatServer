...
final class KnotClassLoader extends AbstractSecureClassLoader implements ClassLoaderAccess {
	...
	KnotClassLoader(boolean isDevelopment, EnvType envType, @Deprecated GameProvider provider) { // CatServer
		super("knot", new DynamicURLClassLoader(new URL[0]));
		this.originalLoader = /*getClass().getClassLoader()*/Thread.currentThread().getContextClassLoader(); // CatServer - TransformingClassLoader
