...
public interface GameProvider { // name directly referenced in net.fabricmc.loader.impl.launch.knot.Knot.findEmbedddedGameProvider() and service loader records
	...
	class BuiltinMod {
		public BuiltinMod(List<Path> paths, ModMetadata metadata) {
			...
			this.type = catserver.server.fabric_loading.FabricModType.UNKNOWN; // CatServer
		}

		// CatServer start
		public BuiltinMod(List<Path> paths, ModMetadata metadata, catserver.server.fabric_loading.FabricModType type) {
			Objects.requireNonNull(paths, "null paths");
			Objects.requireNonNull(metadata, "null metadata");
		...
		public final catserver.server.fabric_loading.FabricModType type; // CatServer
	}
}
