...
public final class DefaultLanguageAdapter implements LanguageAdapter {
	...
	public <T> T create(ModContainer mod, String value, Class<T> type) throws LanguageAdapterException {
		...
		try {
			c = Class.forName(methodSplit[0], true, net.fabricmc.loader.impl.launch.knot.Knot.forgeTransformingClassLoader);
		...
		if (methodSplit.length == 1) {
			...
			try {
				handle = MethodHandles.publicLookup() // CatServer - lookup -> publicLookup. in forge env, MethodHandles.lookup() will cause IllegalAccessException when checking access
