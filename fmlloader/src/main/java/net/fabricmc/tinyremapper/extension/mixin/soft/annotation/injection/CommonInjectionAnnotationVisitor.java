	...
	private static class InjectMethodMappable implements IMappable<MemberInfo> {
		...
		public MemberInfo result() {
			...
			if (collection.size() > 1) {
				...
				// CatServer start
				if (net.fabricmc.loader.impl.discovery.RuntimeModRemapper.isRemapping) {
					data.getLogger().debug(Message.NO_MAPPING_NON_RECURSIVE, info.getName(), targets); // CatServer - warn -> debug because some mods has mixin compat classes for other mc versions
				} else {
