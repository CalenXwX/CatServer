	...
	protected Optional<String> getMappedName() {
		...
		if (collection.size() > 1) {
			...
			if (net.fabricmc.loader.impl.discovery.RuntimeModRemapper.isRemapping) {
				data.getLogger().debug(Message.NO_MAPPING_RECURSIVE, self.getName(), targets); // CatServer - warn -> debug because some mods has mixin compat classes for other mc versions
			} else {
