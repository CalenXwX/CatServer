...
class AsmRemapper extends TrRemapper {
	...
	final String mapMethodName(ClassInstance cls, String name, String desc) {
		...
		// CatServer start - map override methods
		if (name.startsWith("method_")) {
			java.util.List<String> found = tr.methodMap.keySet().stream().filter(m -> m.endsWith(name + desc)).collect(Collectors.toList());
			if (!found.isEmpty()) {
				return tr.methodMap.get(found.get(0));
			}
		}
		// CatServer end
