...
public final class ServerLoginNetworkAddon extends AbstractNetworkAddon<ServerLoginNetworking.LoginQueryResponseHandler> implements PacketSender {
	...
	private final ConcurrentHashMap.KeySetView<Integer, Boolean> ignoredQueries = ConcurrentHashMap.newKeySet(); // SinytraConnector
	...
	private boolean handle(int queryId, @Nullable FriendlyByteBuf originalBuf) {
		...
		// SinytraConnector start
		if (this.ignoredQueries.remove(queryId)) {
			this.logger.debug("login query with id {} doesn't belong to fabric", queryId); // CatServer
			return false;
		}
		// SinytraConnector end
		this.logger.debug("login query with id {} is a fabric query", queryId); // CatServer
	...
	public void registerOutgoingPacket(ClientboundCustomQueryPacket packet) {
		...
		if (ServerNetworkingImpl.LOGIN.getHandler(packet.getIdentifier()) == null) {
			this.ignoredQueries.add(packet.getTransactionId());
			return;
		}
		// SinytraConnector end
